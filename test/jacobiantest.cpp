...
TEST(ConstAcc, PoseInterpolator) {
  ...
  finalicp::traj::Time t1(0.);
  finalicp::traj::Time t2(dt);
  finalicp::traj::Time tau(dt / 2);
  ...
  // check the forward pass is what we expect
  {
    ...
    math::se3::Transformation T_q0_expected =
        math::se3::Transformation(Eigen::Matrix<double, 6, 1>(
            w_01_in1 * (dt / 2) + 0.5 * dw_01_in1 * pow((dt / 2), 2) +
            (1 / 12) * math::se3::curlyhat(dw_01_in1) * w_01_in1 *
                pow((dt / 2), 3) +
            (1 / 240) * math::se3::curlyhat(dw_01_in1) *
                math::se3::curlyhat(dw_01_in1) * w_01_in1 *
                pow((dt / 2), 5))) *
        T_10;
  ...
  // // T1
  {
    ...
    for (int j = 0; j < 6; ++j) {
      ...
      const auto T_10_var_mod1 =
          se3::SE3StateVar::MakeShared(math::se3::Transformation(xi1) * T_10);
      const auto T_10_var_mod2 =
          se3::SE3StateVar::MakeShared(math::se3::Transformation(xi2) * T_10);
  ...
  // T2
  {
    ...
    for (int j = 0; j < 6; ++j) {
      ...
      const auto T_20_var_mod1 =
          se3::SE3StateVar::MakeShared(math::se3::Transformation(xi1) * T_20);
      const auto T_20_var_mod2 =
          se3::SE3StateVar::MakeShared(math::se3::Transformation(xi2) * T_20);
  ...
  // dw2
  {
    ...
    for (int i = 0; i < 6; ++i) {
      for (int j = 0; j < 6; ++j) {
        if (fabs(ajac(i, j)) > 1.0e-2)
          EXPECT_LT(fabs((njac(i, j) - ajac(i, j)) / ajac(i, j)), 0.05);
        else
          EXPECT_LT(fabs(njac(i, j) - ajac(i, j)), 1.0e-2);
      }
    }
...
TEST(ConstAcc, VelocityInterpolator) {
  ...
  // Use Magnus expansion to extrapolate pose using constant acceleration
  math::se3::Transformation T_20 =
      math::se3::Transformation(Eigen::Matrix<double, 6, 1>(
          w_01_in1 * dt + 0.5 * dw_01_in1 * pow(dt, 2) +
          (1 / 12) * math::se3::curlyhat(dw_01_in1) * w_01_in1 * pow(dt, 3) +
          (1 / 240) * math::se3::curlyhat(dw_01_in1) *
              math::se3::curlyhat(dw_01_in1) * w_01_in1 * pow(dt, 5))) *
      T_10;
...
TEST(Singer, PoseInterpolator) {
  using namespace finalicp::traj::singer;
  using namespace finalicp;
  // TODO: try a bunch of random dw_01_in1;
  const Eigen::Matrix<double, 6, 1> dw_01_in1 =
      -1 * Eigen::Matrix<double, 6, 1>::Ones();
...
TEST(Singer, VelocityInterpolator) {
  ...
  const auto w_0q_inq_eval = VelocityInterpolator::MakeShared(tau, knot1, knot2, ad);
...
TEST(IMU, AccelerationErrorEvaluator) {
  ...
  math::se3::Transformation T_vm(xi_vm);

  Eigen::Matrix<double, 6, 1> xi_mi;
  xi_mi << 0, 0, 0, 0.2, 0.2, 0;
  math::se3::Transformation T_mi(xi_mi);
  ...
  // T_vm
  {
    ...
    for (int j = 0; j < 6; ++j) {
      ...
      const auto T_vm_var_mod1 =
          se3::SE3StateVar::MakeShared(math::se3::Transformation(xi1) * T_vm);
      const auto T_vm_var_mod2 =
          se3::SE3StateVar::MakeShared(math::se3::Transformation(xi2) * T_vm);
  ...
  // T_mi
  {
    ...
    for (int j = 0; j < 6; ++j) {
      ...
      const auto T_mi_var_mod1 =
          se3::SE3StateVar::MakeShared(math::se3::Transformation(xi1) * T_mi);
      const auto T_mi_var_mod2 =
          se3::SE3StateVar::MakeShared(math::se3::Transformation(xi2) * T_mi);
      const auto acc_err_eval_mod1 = AccelerationError(
...
TEST(ConstVel, VelocityInterpolator) {
  using namespace finalicp::traj::const_vel;
  using namespace finalicp;
  ...
  math::se3::Transformation T_20 =
      math::se3::Transformation(Eigen::Matrix<double, 6, 1>(
          ...
          (1 / 12) * math::se3::curlyhat(accel) * w_01_in1 * pow(dt, 3) +
          (1 / 240) * math::se3::curlyhat(accel) *
              math::se3::curlyhat(accel) * w_01_in1 * pow(dt, 5))) *
      T_10;
  ...
  const auto w_0q_inq_eval = VelocityInterpolator::MakeShared(tau, knot1, knot2);
  ...
  // T1
  {
    ...
    for (int j = 0; j < 6; ++j) {
      ...
      const auto knot1_mod1 = std::make_shared<Variable>(t1, T_10_var_mod1, w_01_in1_var);
      const auto knot1_mod2 = std::make_shared<Variable>(t1, T_10_var_mod2, w_01_in1_var);

      const auto w_0q_inq_eval_mod1 = VelocityInterpolator::MakeShared(tau, knot1_mod1, knot2);
      const auto w_0q_inq_eval_mod2 = VelocityInterpolator::MakeShared(tau, knot1_mod2, knot2);
      njac.block<6, 1>(0, j) = (w_0q_inq_eval_mod1->evaluate() - w_0q_inq_eval_mod2->evaluate()) / (2 * eps);
    }
    std::cout << "Numerical:\n" << njac << std::endl;
    // EXPECT_LT((njac - ajac).norm(), 1e-2);
    for (int i = 0; i < 6; ++i) {
  ...
  // w1
  {
    ...
    for (int j = 0; j < 6; ++j) {
      ...
      const auto w_01_in1_var_mod1 = vspace::VSpaceStateVar<6>::MakeShared(w_01_in1 + xi1);
      const auto w_01_in1_var_mod2 = vspace::VSpaceStateVar<6>::MakeShared(w_01_in1 + xi2);
      const auto knot1_mod1 = std::make_shared<Variable>(t1, T_10_var, w_01_in1_var_mod1);
      const auto knot1_mod2 = std::make_shared<Variable>(t1, T_10_var, w_01_in1_var_mod2);
  ...
  // T2
  {
    ...
    for (int j = 0; j < 6; ++j) {
      ...
      const auto T_20_var_mod1 = se3::SE3StateVar::MakeShared(math::se3::Transformation(xi1) * T_20);
      const auto T_20_var_mod2 = se3::SE3StateVar::MakeShared(math::se3::Transformation(xi2) * T_20);
      const auto knot2_mod1 = std::make_shared<Variable>(t2, T_20_var_mod1, w_02_in2_var);
      const auto knot2_mod2 = std::make_shared<Variable>(t2, T_20_var_mod2, w_02_in2_var);

      const auto w_0q_inq_eval_mod1 = VelocityInterpolator::MakeShared(tau, knot1, knot2_mod1);
      const auto w_0q_inq_eval_mod2 = VelocityInterpolator::MakeShared(tau, knot1, knot2_mod2);
      njac.block<6, 1>(0, j) = (w_0q_inq_eval_mod1->evaluate() - w_0q_inq_eval_mod2->evaluate()) / (2 * eps);
    }
    std::cout << "Numerical:\n" << njac << std::endl;
    // EXPECT_LT((njac - ajac).norm(), 1e-2);
  ...
  // w2
  {
    ...
    for (int j = 0; j < 6; ++j) {
      ...
      const auto knot2_mod1 = std::make_shared<Variable>(t2, T_20_var, w_02_in2_var_mod1);
      const auto knot2_mod2 = std::make_shared<Variable>(t2, T_20_var, w_02_in2_var_mod2);
...
TEST(IMU, PreintIMUCostTerm) {
  ...
  Eigen::Matrix3d C_ij = math::so3::vec2rot(phi);
  ...
  for (int i = 1; i < 10; ++i) {
    ...
    C_ij = C_ij * math::so3::vec2rot(delta_t * ang_vel);
  }
  r_ij += v_ij * delta_t + 0.5 * ((C_ij * lin_acc) + gravity) * delta_t * delta_t;
  v_ij += ((C_ij * lin_acc) + gravity) * delta_t;
  C_ij = C_ij * math::so3::vec2rot(delta_t * ang_vel);
  ...
  T_i.block<3, 3>(0, 0) = math::so3::vec2rot(phi);
  auto transform_i = math::se3::Transformation(T_i);
  ...
  auto transform_j = math::se3::Transformation(T_j);
  Eigen::Vector3d v_j = v_ij;

  Eigen::Matrix<double, 6, 1> bias = Eigen::Matrix<double, 6, 1>::Zero();

  const auto T_i_var = se3::SE3StateVarGlobalPerturb::MakeShared(math::se3::Transformation(T_i));
  const auto v_i_var = vspace::PreIntVelocityStateVar<3>::MakeShared(v_i, T_i_var);
  const auto T_j_var = se3::SE3StateVarGlobalPerturb::MakeShared(math::se3::Transformation(T_j));
  ...
  // T_i
  {
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      const auto T_i_var_mod1 = se3::SE3StateVarGlobalPerturb::MakeShared(math::se3::Transformation(T_1));
      const auto T_i_var_mod2 = se3::SE3StateVarGlobalPerturb::MakeShared(math::se3::Transformation(T_2));
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      T_1.block<3, 3>(0, 0) = math::so3::vec2rot(xi1);
      Eigen::Matrix4d T_2 = Eigen::Matrix4d::Identity();
      T_2.block<3, 3>(0, 0) = math::so3::vec2rot(xi2);
      const auto T_i_var_mod1 = se3::SE3StateVarGlobalPerturb::MakeShared(T_i * math::se3::Transformation(T_1));
      const auto T_i_var_mod2 = se3::SE3StateVarGlobalPerturb::MakeShared(T_i * math::se3::Transformation(T_2));
      ...
      njac.block<9, 1>(0, 3 + j) = (preint_cost_term_mod1->get_error() - preint_cost_term_mod2->get_error()) / (2 * eps);
    }
  ...
  // T_j
  {
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      const auto T_j_var_mod1 = se3::SE3StateVarGlobalPerturb::MakeShared(math::se3::Transformation(T_1));
      const auto T_j_var_mod2 = se3::SE3StateVarGlobalPerturb::MakeShared(math::se3::Transformation(T_2));
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      Eigen::Matrix4d T_2 = Eigen::Matrix4d::Identity();
      T_2.block<3, 3>(0, 0) = math::so3::vec2rot(xi2);
      const auto T_j_var_mod1 = se3::SE3StateVarGlobalPerturb::MakeShared(T_j * math::se3::Transformation(T_1));
      const auto T_j_var_mod2 = se3::SE3StateVarGlobalPerturb::MakeShared(T_j * math::se3::Transformation(T_2));
...
TEST(P2P, P2PGlobal) {
  using namespace finalicp;
  using namespace finalicp::p2p;
  
  Eigen::Matrix<double, 6, 1> xi;
  xi << 1, 2, 3, 0.5, 0.9, 1.2;
  const auto T_iv = math::se3::Transformation(xi);
  ...
  const double eps = 1.0e-5;
  Jacobians jacs;
  Eigen::Matrix<double, 1, 1> lhs = Eigen::Matrix<double, 1, 1>::Identity(); // Fixed
  ...
  // T_iv
  {
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      const auto T_iv_var_mod1 =
          se3::SE3StateVarGlobalPerturb::MakeShared(math::se3::Transformation(T_1));
      const auto T_iv_var_mod2 =
          se3::SE3StateVarGlobalPerturb::MakeShared(math::se3::Transformation(T_2));
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      T_2.block<3, 3>(0, 0) = math::so3::vec2rot(xi2);
      const auto T_iv_var_mod1 =
          se3::SE3StateVarGlobalPerturb::MakeShared(T_iv * math::se3::Transformation(T_1));
      const auto T_iv_var_mod2 =
          se3::SE3StateVarGlobalPerturb::MakeShared(T_iv * math::se3::Transformation(T_2));
...
TEST(P2P, P2PGlobalSuper) {
  ...
  const Eigen::Matrix3d C_0 = math::so3::vec2rot(phi_i);
  ...
  for (int i = 1; i < N; ++i) {
    ...
    C_mr = C_mr * math::so3::vec2rot(delta_t * (ang_vel - bg));
  ...
  // check the integration is what we expect
  {
    for (size_t i = 0; i < states.size(); ++i) {
      ...
      Eigen::Vector3d error = math::so3::rot2vec(states_out[i].C_mr * states[i].C_mr.transpose());
  ...
  for (int jac_check_index = 0; jac_check_index < (int)states.size(); ++jac_check_index) {
    ...
    const Eigen::Matrix<double, 6, 15> jac = states_out[jac_check_index].jacobian;
    {
      ...
      for (int j = 0; j < 3; ++j) {
        ...
        // rot
        njac.block<3, 1>(3, j) = (math::so3::rot2vec(states[jac_check_index].C_mr.transpose() * states_mod1[jac_check_index].C_mr) - math::so3::rot2vec(states[jac_check_index].C_mr.transpose() * states_mod2[jac_check_index].C_mr)) / (2 * eps);
      }
      std::cout << "Numerical:\n" << njac << std::endl;
      for (int i = 0; i < njac.rows(); ++i) {
        for (int j = 0; j < njac.cols(); ++j) {
    ...
    }
    {
      ...
      for (int j = 0; j < 3; ++j) {
        ...
        T_2.block<3, 3>(0, 0) = math::so3::vec2rot(xi2);
        const auto T_i_var_mod1 =
            se3::SE3StateVarGlobalPerturb::MakeShared(T_i * math::se3::Transformation(T_1));
        const auto T_i_var_mod2 =
            se3::SE3StateVarGlobalPerturb::MakeShared(T_i * math::se3::Transformation(T_2));
