...
class MotionError : public Evaluable<Eigen::Matrix<double, 1, 1>> {
 public:
  ...
  void backward(const Eigen::MatrixXd& lhs, const Node<OutType>::Ptr& node, Jacobians& jacs) const {
    if (xkm1_->active()) {
        const auto xkm1_node = std::static_pointer_cast<Node<InType>>(node->at(0));
        xkm1_->backward(-lhs, xkm1_node, jacs);
    }
    if (xk_->active()) {
        const auto xk_node = std::static_pointer_cast<Node<InType>>(node->at(1)); // Fix: Use index 1
...
int main(int argc, char** argv) {
  ...
  const auto noise_model = finalicp::StaticNoiseModel<1>::MakeShared(Cov{x0_cov});
  ...
  for (int k = 1; k < (int)v.size(); ++k) {
    ...
    {
      const auto loss_func = L2LossFunc::MakeShared();
      const auto noise_model = finalicp::StaticNoiseModel<1>::MakeShared(Cov{v_cov});
    ...
    {
      const auto loss_func = L2LossFunc::MakeShared();
      const auto noise_model = finalicp::StaticNoiseModel<1>::MakeShared(Cov{r_cov});
    ...
    //
    std::cout << "[OUTPUT::Sample_SlidingWindow] x" << k << " = " << xk_var->value().value() << std::endl;
  }
