...
using SE3StateVar = finalicp::se3::SE3StateVar;
...
void runPoseGraphRelax() {
  ...
  const auto noise_model = finalicp::StaticNoiseModel<6>::MakeShared(cov);
  const auto loss_function = finalicp::L2LossFunc::MakeShared();
  ...
  for (unsigned int i = 0; i < measCollection.size(); i++) {
    ...
    // Construct cost term
    const auto cost_term = finalicp::WeightedLeastSqCostTerm<6>::MakeShared(
  ...
  ///
  finalicp::GaussNewtonSolver::Params params;
  params.verbose = true;
  finalicp::GaussNewtonSolver solver(problem, params);
