...
namespace finalicp {
    namespace traj {
        namespace bspline {
            ...
            VelocityInterpolator::Ptr VelocityInterpolator::MakeShared(const Time& time, const Variable::ConstPtr& k1,
            ...
            bool VelocityInterpolator::active() const {
                return k1_->getC()->active() || k2_->getC()->active() || k3_->getC()->active() || k4_->getC()->active();
            }
            ...
            auto VelocityInterpolator::forward() const -> Node<OutType>::Ptr {
                ...
                const auto value = w_(0) * k1->value() + w_(1) * k2->value() + w_(2) * k3->value() + w_(3) * k4->value();
