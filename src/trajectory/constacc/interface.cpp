...
// #endif
            ...
            auto Interface::getCovariance(const Covariance& cov, const Time time) -> CovType {
                ...
                if (!T_10->active() || !w_01_in1->active() || !dw_01_in1->active() || !T_20->active() || !w_02_in2->active() || !dw_02_in2->active()) 
                throw std::runtime_error("extrapolation from a locked knot not implemented.");
                ...
                const Eigen::Matrix<double, 18, 18> P_t_inv = E_t1.transpose() * Qt1_inv * E_t1 + F_2t.transpose() * Q2t_inv * F_2t - A.transpose() * (P_1n2.inverse() + B - Pinv_comp).inverse() * A;
            ...
            void Interface::addStatePrior(const Time time, const PoseType& T_k0, const VelocityType& w_0k_ink, const AccelerationType& dw_0k_ink, const CovType& cov) {
                ...
                if ((pose_prior_factor_ != nullptr) || (vel_prior_factor_ != nullptr) || (acc_prior_factor_ != nullptr))
                ...
                // Check that the pose is not locked
                if ((!knot->pose()->active()) || (!knot->velocity()->active()) || (!knot->acceleration()->active()))
            ...
            auto Interface::getPoseInterpolator_(const Time time, const Variable::ConstPtr& knot1, const Variable::ConstPtr& knot2) const -> Evaluable<PoseType>::Ptr {
            ...
            auto Interface::getPoseExtrapolator_(const Time time, const Variable::ConstPtr& knot) const -> Evaluable<PoseType>::Ptr {
