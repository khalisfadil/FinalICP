        ...
        namespace const_acc {
            ...
            VelocityInterpolator::VelocityInterpolator(const Time time, const Variable::ConstPtr& knot1, const Variable::ConstPtr& knot2)
...
#ifdef DEBUG
                std::cout << "[CONSTACC VelocityInterpolator DEBUG] interpolating velocity with const_acc model. Interval T: " << T << "s, at tau: " << tau << "s." << std::endl;
                if (T <= 0) {
                    std::cerr << "[CONSTACC VelocityInterpolator DEBUG] CRITICAL: Total time interval T is zero or negative!" << std::endl;
                }
#endif
                ...
                if (!omega_.allFinite() || !lambda_.allFinite()) {
                    std::cerr << "[CONSTACC VelocityInterpolator DEBUG] CRITICAL: Final interpolation matrices (omega/lambda) contain non-finite values!" << std::endl;
                }
#endif
            }
            ...
            auto VelocityInterpolator::value() const -> OutType {
                ...
                if (!w1.allFinite() || !dw1.allFinite() || !w2.allFinite() || !dw2.allFinite()) {
                    std::cerr << "[CONSTACC VelocityInterpolator DEBUG | value] CRITICAL: Input knot states are non-finite!" << std::endl;
                }
#endif
                ...
                #ifdef DEBUG
                    if (!xi_i1.allFinite() || !xi_j1.allFinite()) {
                        std::cerr << "[CONSTACC VelocityInterpolator DEBUG | value] CRITICAL: Intermediate xi vectors are non-finite!" << std::endl;
                    }
                #endif
                // Calculate interpolated velocity
                OutType w_i = math::se3::vec2jac(xi_i1) * xi_j1;
#ifdef DEBUG
                if (!w_i.allFinite()) {
                    std::cerr << "[CONSTACC VelocityInterpolator DEBUG | value] CRITICAL: Final interpolated velocity is non-finite!" << std::endl;
                }
#endif
                return w_i;
            }
            ...
            void VelocityInterpolator::backward(const Eigen::MatrixXd& lhs, const Node<OutType>::Ptr& node, Jacobians& jacs) const {
...
#ifdef DEBUG
                if (!lhs.allFinite()) {
                    std::cerr << "[CONSTACC VelocityInterpolator DEBUG | backward] CRITICAL: Incoming derivative (lhs) is non-finite!" << std::endl;
                }
#endif
                const auto T1 = knot1_->pose()->value();
                const auto w1 = knot1_->velocity()->value();
                ...
                if (knot1_->pose()->active() || knot2_->pose()->active()) {
                    ...
                    const Eigen::Matrix<double, 6, 6> w = J_i1 * (omega_.block<6, 6>(6, 0) * Eigen::Matrix<double, 6, 6>::Identity() + omega_.block<6, 6>(6, 6) * 0.5 * math::se3::curlyhat(w2) +
                            omega_.block<6, 6>(6, 12) * 0.25 * math::se3::curlyhat(w2) * math::se3::curlyhat(w2) + omega_.block<6, 6>(6, 12) * 0.5 * math::se3::curlyhat(dw2)) * J_21_inv + xi_j1_ch *
                            (omega_.block<6, 6>(0, 0) * Eigen::Matrix<double, 6, 6>::Identity() + omega_.block<6, 6>(0, 6) * 0.5 * math::se3::curlyhat(w2) +
                            omega_.block<6, 6>(0, 12) * 0.25 * math::se3::curlyhat(w2) * math::se3::curlyhat(w2) + omega_.block<6, 6>(0, 12) * 0.5 * math::se3::curlyhat(dw2)) * J_21_inv;
...
#ifdef DEBUG
                        if (!new_lhs.allFinite()) std::cerr << "[CONSTACC VelocityInterpolator DEBUG | backward] CRITICAL: Derivative to T1 is non-finite!" << std::endl;
#endif
...
#ifdef DEBUG
                        if (!new_lhs.allFinite()) std::cerr << "[CONSTACC VelocityInterpolator DEBUG | backward] CRITICAL: Derivative to T2 is non-finite!" << std::endl;
#endif
                ...
                if (knot1_->velocity()->active()) {
                    ...
                    Eigen::MatrixXd new_lhs = lhs * (J_i1 * lambda_.block<6, 6>(6, 6) + xi_j1_ch * lambda_.block<6, 6>(0, 6));
#ifdef DEBUG
                    if (!new_lhs.allFinite()) std::cerr << "[CONSTACC VelocityInterpolator DEBUG | backward] CRITICAL: Derivative to w1 is non-finite!" << std::endl;
#endif
                ...
                if (knot2_->velocity()->active()) {
                    ...
                    Eigen::MatrixXd new_lhs = lhs * (J_i1 * (omega_.block<6, 6>(6, 6) * J_21_inv + omega_.block<6, 6>(6, 12) * -0.5 *
                                        (math::se3::curlyhat(J_21_inv * w2) - math::se3::curlyhat(w2) * J_21_inv)) + xi_j1_ch * (omega_.block<6, 6>(0, 6) * J_21_inv +
                                        omega_.block<6, 6>(0, 12) * -0.5 * (math::se3::curlyhat(J_21_inv * w2) - math::se3::curlyhat(w2) * J_21_inv)));
#ifdef DEBUG
                    if (!new_lhs.allFinite()) std::cerr << "[CONSTACC VelocityInterpolator DEBUG | backward] CRITICAL: Derivative to w2 is non-finite!" << std::endl;
#endif
...
#ifdef DEBUG
                    if (!new_lhs.allFinite()) std::cerr << "[CONSTACC VelocityInterpolator DEBUG | backward] CRITICAL: Derivative to dw1 is non-finite!" << std::endl;
#endif
                ...
                if (knot2_->acceleration()->active()) {
                    ...
                    Eigen::MatrixXd new_lhs = lhs * (J_i1 * (omega_.block<6, 6>(6, 12) * J_21_inv) + xi_j1_ch * (omega_.block<6, 6>(0, 12) * J_21_inv));
#ifdef DEBUG
                    if (!new_lhs.allFinite()) std::cerr << "[CONSTACC VelocityInterpolator DEBUG | backward] CRITICAL: Derivative to dw2 is non-finite!" << std::endl;
#endif
