...
namespace finalicp {
    namespace traj {
        namespace const_vel {
            ...
            void Interface::add(const Time time, const Evaluable<PoseType>::Ptr& T_k0, const Evaluable<VelocityType>::Ptr& w_0k_ink) {
            ...
            auto Interface::getCovariance(const Covariance& cov, const Time time) -> CovType {
                ...
                const Eigen::Matrix<double, 12, 12> P_t_inv = E_t1.transpose() * Qt1_inv * E_t1 + F_2t.transpose() * Q2t_inv * F_2t - A.transpose() * (P_1n2.inverse() + B - Pinv_comp).inverse() * A;
                ...
                P_1n2_k.block<12, 12>(0, 0) = G_1 * (P_1n2.block<12, 12>(0, 0) - Xi_1 * P_1n2.block<12, 12>(0, 0) * Xi_1.transpose()) * G_1.transpose();
                P_1n2_k.block<12, 12>(0, 12) = G_1 * (P_1n2.block<12, 12>(0, 12) - Xi_1 * P_1n2.block<12, 12>(0, 0) * Xi_2.transpose()) * G_2.transpose();
                P_1n2_k.block<12, 12>(12, 0) = P_1n2_k.block<12, 12>(0, 12).transpose();
                P_1n2_k.block<12, 12>(12, 12) = G_2 * (P_1n2.block<12, 12>(12, 12) - Xi_2 * P_1n2.block<12, 12>(0, 0) * Xi_2.transpose()) * G_2.transpose();
                ...
                P_tau = G_tau_inv * P_k_tau * G_tau_inv.transpose() + Xi_tau * P_1n2.block<12, 12>(0, 0) * Xi_tau.transpose();
            ...
            void Interface::addPriorCostTerms(Problem& problem) const {
                ...
                for (; it2 != knot_map_.end(); ++it1, ++it2) {
                    ...
                    if (knot1->pose()->active() || knot1->velocity()->active() ||
                        ...
                        const auto noise_model = std::make_shared<StaticNoiseModel<12>>(Qinv, NoiseType::INFORMATION);
                        //
                        const auto error_function = PriorFactor::MakeShared(knot1, knot2);
                        // Create cost term
                        const auto cost_term = std::make_shared<WeightedLeastSqCostTerm<12>>(error_function, noise_model, loss_function);
                        //
