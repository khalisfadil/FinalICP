...
namespace finalicp {
    ...
    bool DoglegGaussNewtonSolver::linearizeSolveAndUpdate(double& cost, double& grad_norm) {
        ...
        Eigen::VectorXd grad_descent_step = getCauchyPoint(approximate_hessian, gradient_vector);
        ...
        for (; num_backtrack < params_.max_shrink_steps; num_backtrack++) {
            ...
            if (gauss_newton_norm <= trust_region_size_) {
                ...
                dogleg_step = (trust_region_size_ / grad_descent_norm) * grad_descent_step;
                ...
                double interp_const =(-gd_dot_gd2gn + std::sqrt(gd_dot_gd2gn * gd_dot_gd2gn + (trust_region_size_ * trust_region_size_ - grad_descent_norm * grad_descent_norm) * gd2gn_sqnorm)) / gd2gn_sqnorm;
            ...
            double predicted_reduc = predictedReduction(approximate_hessian, gradient_vector, dogleg_step);
    ...
    Eigen::VectorXd DoglegGaussNewtonSolver::getCauchyPoint(const Eigen::SparseMatrix<double>& approximate_hessian, const Eigen::VectorXd& gradient_vector) {
    ...
    double DoglegGaussNewtonSolver::predictedReduction(const Eigen::SparseMatrix<double>& approximate_hessian, const Eigen::VectorXd& gradient_vector, const Eigen::VectorXd& step) {
