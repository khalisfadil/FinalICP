...
namespace finalicp {
    ...
    LevMarqGaussNewtonSolver::LevMarqGaussNewtonSolver(Problem& problem, const Params& params)
    ...
    bool LevMarqGaussNewtonSolver::linearizeSolveAndUpdate(double& cost, double& grad_norm) {
        ...
        for (; num_backtrack < params_.max_shrink_steps; num_backtrack++) {
            ...
            if (decomp_success) {
                ...
                double predicted_reduc = predictedReduction(approximate_hessian, gradient_vector, lev_marq_step);
    ...
    Eigen::VectorXd LevMarqGaussNewtonSolver::solveLevMarq(Eigen::SparseMatrix<double>& approximate_hessian, const Eigen::VectorXd& gradient_vector, double diagonal_coeff) {
    ...
    double LevMarqGaussNewtonSolver::predictedReduction(const Eigen::SparseMatrix<double>& approximate_hessian, const Eigen::VectorXd& gradient_vector, const Eigen::VectorXd& step) {
        // grad^T * step - 0.5 * step^T * Hessian * step
        double grad_tran_step = gradient_vector.transpose() * step;
        double step_trans_hessian_step = step.transpose() * (approximate_hessian.selfadjointView<Eigen::Upper>() * step);
