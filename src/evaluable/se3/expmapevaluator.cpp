#include <evaluable/se3/expmapevaluator.hpp>
...
namespace finalicp {
    namespace se3 {
        ...
        void ExpMapEvaluator::backward(const Eigen::MatrixXd &lhs,
            ...
            if (xi_->active()) {
                Eigen::MatrixXd new_lhs = lhs * math::se3::vec2jac(node->value().vec());
                xi_->backward(new_lhs, std::static_pointer_cast<Node<InType>>(node->at(0)),jacs);
            }
        }
