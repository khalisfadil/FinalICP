#include <evaluable/se3/se3globalperturbevaluator.hpp>

namespace finalicp {
    namespace se3 {
        auto SE3ErrorGlobalPerturbEvaluator::MakeShared(const Evaluable<InType>::ConstPtr &T_ab, const InType &T_ab_meas) -> Ptr {
            return std::make_shared<SE3ErrorGlobalPerturbEvaluator>(T_ab, T_ab_meas);
        }

        SE3ErrorGlobalPerturbEvaluator::SE3ErrorGlobalPerturbEvaluator(const Evaluable<InType>::ConstPtr &T_ab, const InType &T_ab_meas)
        ...
        auto SE3ErrorGlobalPerturbEvaluator::value() const -> OutType {
            ...
            out.block<3, 1>(3, 0) = math::so3::rot2vec(T_ab_meas.block<3, 3>(0, 0).transpose() * T_ab.block<3, 3>(0, 0));
            return out;
        }
        ...
        auto SE3ErrorGlobalPerturbEvaluator::forward() const -> Node<OutType>::Ptr {
            ...
            value.block<3, 1>(3, 0) = math::so3::rot2vec(T_ab_meas.block<3, 3>(0, 0).transpose() * T_ab.block<3, 3>(0, 0));
        ...
        void SE3ErrorGlobalPerturbEvaluator::backward(const Eigen::MatrixXd &lhs,
            ...
            if (T_ab_->active()) {
                ...
                jac.block<3, 3>(3, 3) = math::so3::vec2jacinv(-phi);
                T_ab_->backward(lhs * jac, child, jacs);
            }
        }

        SE3ErrorGlobalPerturbEvaluator::Ptr se3_global_perturb_error(const Evaluable<SE3ErrorGlobalPerturbEvaluator::InType>::ConstPtr &T_ab, const SE3ErrorGlobalPerturbEvaluator::InType &T_ab_meas) {
