#include <evaluable/imu/imuerrorevaluator.hpp>
...
namespace finalicp {
    namespace imu {
        ...
        bool IMUErrorEvaluator::active() const {
            return transform_->active() || velocity_->active() || acceleration_->active() || bias_->active() || transform_i_to_m_->active();
        }
        ...
        void IMUErrorEvaluator::backward(const Eigen::MatrixXd &lhs, const Node<OutType>::Ptr &node, Jacobians &jacs) const {
            ...
            if (transform_i_to_m_->active()) {
                Eigen::Matrix<double, 6, 6> jac = Eigen::Matrix<double, 6, 6>::Zero();
                jac.block<3, 3>(0, 3) = -1 * child1->value().C_ba() * math::so3::hat(child5->value().C_ba() * gravity_);
        ...
        IMUErrorEvaluator::Ptr imuError(const Evaluable<IMUErrorEvaluator::PoseInType>::ConstPtr &transform,
            ...
            return IMUErrorEvaluator::MakeShared(transform, velocity, acceleration, bias, transform_i_to_m, imu_meas);
        }
    }  // namespace imu
}  // namespace finalicp
