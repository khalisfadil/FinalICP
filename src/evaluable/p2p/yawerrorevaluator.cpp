#include <evaluable/p2p/yawerrorevaluator.hpp>

namespace finalicp {
    namespace p2p {
        auto YawErrorEvaluator::MakeShared(const double yaw_meas,
        ...
        YawErrorEvaluator::YawErrorEvaluator(const double yaw_meas,
                                                const Evaluable<PoseInType>::ConstPtr &T_ms_prev,
        ...
        auto YawErrorEvaluator::value() const -> OutType {
            ...
            const math::so3::Rotation C_pc_meas(meas_vec);
            const math::so3::Rotation C_cp_eval((T_ms_curr_->value().C_ba().inverse() * 
                                                            T_ms_prev_->value().C_ba()).eval());
            // Return error
            return d_ * math::so3::rot2vec((C_cp_eval * C_pc_meas).matrix());
        }
        ...
        auto YawErrorEvaluator::forward() const -> Node<OutType>::Ptr {
            ...
            Eigen::Vector3d meas_vec(0.0, 0.0, yaw_meas_);
            const math::so3::Rotation C_pc_meas(meas_vec);
            const math::so3::Rotation C_cp_eval((C_ms_curr.transpose() * C_ms_prev).eval());
            OutType error = d_ * math::so3::rot2vec((C_cp_eval * C_pc_meas).matrix());
            // clang-format on
        ...
        void YawErrorEvaluator::backward(const Eigen::MatrixXd &lhs, const Node<OutType>::Ptr &node, Jacobians &jacs) const {
