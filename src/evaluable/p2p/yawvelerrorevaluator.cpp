#include <evaluable/p2p/yawvelerrorevaluator.hpp>

namespace finalicp {
    namespace p2p {
        auto YawVelErrorEvaluator::MakeShared(const Eigen::Matrix<double, 1, 1> vel_meas, const Evaluable<InType>::ConstPtr &w_iv_inv) -> Ptr {
            return std::make_shared<YawVelErrorEvaluator>(vel_meas, w_iv_inv);
        }

        YawVelErrorEvaluator::YawVelErrorEvaluator(const Eigen::Matrix<double, 1, 1> vel_meas, const Evaluable<InType>::ConstPtr &w_iv_inv)
