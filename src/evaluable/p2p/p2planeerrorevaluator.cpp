#include <evaluable/p2p/p2planeerrorevaluator.hpp>
...
namespace finalicp {
    namespace p2p {
        ...
        void P2PlaneErrorEvaluator::backward(const Eigen::MatrixXd &lhs, const Node<OutType>::Ptr &node, Jacobians &jacs) const {
            if (T_rq_->active()) {
                ...
                Eigen::Matrix<double, 1, 6> new_lhs = -lhs * normal_.transpose() * math::se3::point2fs(Tq).block<3, 6>(0, 0);
        ...
        Eigen::Matrix<double, 1, 6> P2PlaneErrorEvaluator::getJacobianPose() const {
            const auto T_rq = T_rq_->value().matrix();
            Eigen::Matrix<double, 3, 1> Tq = T_rq.block<3, 3>(0, 0) * query_ + T_rq.block<3, 1>(0, 3);
            Eigen::Matrix<double, 1, 6> jac = -normal_.transpose() * math::se3::point2fs(Tq).block<3, 6>(0, 0);
            return jac;
        }
