...
namespace finalicp {
    namespace traj {
        namespace const_acc {
            ...
            inline Eigen::Matrix<double, 18, 18> getQinv(const double& dt, const Eigen::Matrix<double, 6, 1>& Qc_diag) {
                // constants
            ...
            inline Eigen::Matrix<double, 18, 18> getQ(const double& dt, const Eigen::Matrix<double, 6, 1>& Qc_diag) {
                // constants
            ...
            inline Eigen::Matrix<double, 18, 18> getJacKnot1(const Variable::ConstPtr& knot1, const Variable::ConstPtr& knot2) {
                // precompute
                ...
                jacobian.block<6, 6>(6, 0) = -0.5 * math::se3::curlyhat(w2) * Jinv_12;
                jacobian.block<6, 6>(12, 0) =
                    -0.25 * math::se3::curlyhat(w2) * math::se3::curlyhat(w2) * Jinv_12 -
                    0.5 * math::se3::curlyhat(dw2) * Jinv_12;
                // velocity
            ...
            inline Eigen::Matrix<double, 18, 18> getJacKnot2(const Variable::ConstPtr& knot1, const Variable::ConstPtr& knot2) {
                // precompute
                const auto T_21 = knot2->pose()->value() / knot1->pose()->value();
                const auto xi_21 = T_21.vec();
                const auto J_21_inv = math::se3::vec2jacinv(xi_21);
                ...
                jacobian.block<6, 6>(6, 0) = 0.5 * math::se3::curlyhat(w2) * J_21_inv;
                jacobian.block<6, 6>(12, 0) =
                    0.25 * math::se3::curlyhat(w2) * math::se3::curlyhat(w2) * J_21_inv +
                    0.5 * math::se3::curlyhat(dw2) * J_21_inv;
                // velocity
                jacobian.block<6, 6>(6, 6) = J_21_inv;
                jacobian.block<6, 6>(12, 6) = -0.5 * math::se3::curlyhat(J_21_inv * w2) +
                                                0.5 * math::se3::curlyhat(w2) * J_21_inv;
                // acceleration
