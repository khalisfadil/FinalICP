...
namespace finalicp {
    namespace traj {
       ...
       class AccelerationInterpolator: public traj::const_acc::AccelerationInterpolator {
            public:
                ...
                AccelerationInterpolator(const Time time, const Variable::ConstPtr& knot1,
                    ...
                    : traj::const_acc::AccelerationInterpolator(time, knot1, knot2) {
                    ...
                    if (T <= 0) {
                        std::cerr << "[SINGER AccelerationInterpolator DEBUG] CRITICAL: Total time interval T is zero or negative!" << std::endl;
                    }
#endif
                    ...
                    if (!lu.isInvertible()) {
                        std::cerr << "[SINGER AccelerationInterpolator DEBUG] CRITICAL: Process noise matrix Q_T is not invertible! Cannot compute interpolation matrices." << std::endl;
                    ...
                    if (!omega_.allFinite() || !lambda_.allFinite()) {
                        std::cerr << "[SINGER AccelerationInterpolator DEBUG] CRITICAL: Final interpolation matrices (omega/lambda) contain non-finite values!" << std::endl;
                    } else {
